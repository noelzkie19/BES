...
namespace BES.Application.JobAggregate.Queries.GetPendingJobPagination;

public class GetPendingJobPagination : IRequest<PaginatedList<JobDto>>
{
...
public class GetPendingJobPaginationHandler : IRequestHandler<GetPendingJobPagination, PaginatedList<JobDto>>
{
    private readonly IApplicationDbContext _context;
    private readonly IMapper _mapper;
    public GetPendingJobPaginationHandler(IApplicationDbContext context, IMapper mapper)
    {
        _context = context;
        _mapper = mapper;
    }
    
    public async Task<PaginatedList<JobDto>> Handle(GetPendingJobPagination request, CancellationToken cancellationToken)
    {
        try 
        {
            var DueDate = "";
            ...
            .Where(item => string.IsNullOrEmpty(request.Search) 
                    ...
                    || item.Job.OrderNumber.StartsWith(request.OrderNumber)
            ...
            .PaginatedListGridAsync(request.Skip,
             request.Take);
